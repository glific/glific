defmodule Glific.Flows.Webhooks.ResultTranslator do
  ...
  consumed by `Glific.Flows.Webhook.handle/3`.

  Until `handle/3` routes on application-level success (not `is_map/1`):
  ...
  * other return values pass through unchanged (legacy map responses)
  ...
  Remove once all webhooks are migrated and `handle/3` is updated.
  """
  ...
  @doc """
  Translates a `call/2` return value into the legacy format for `handle/3`.
