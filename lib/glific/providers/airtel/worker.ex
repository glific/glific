defmodule Glific.Providers.Airtel.Worker do
  ...
  alias Glific.{
    ...
    Providers.Airtel.ResponseHandler,
    Providers.Worker
  }
    ...
    case ExRated.check_rate(
      ...
      {:ok, _} ->
        if Contacts.is_simulator_contact?(payload["destination"]) do
          ...
          process_airtel(organization.id, payload, message, attrs)
        end
  ...
  @spec process_airtel(non_neg_integer(), map(), Message.t(), map()) ::
  ...
  defp process_airtel(org_id, payload, message, _attrs) do
