defmodule Glific.ThirdParty.Kaapi.SttTtsWorker do
  ...
  defp wake_flow_with_failure(context_id, organization_id) do
    ...
    case FlowContext.wakeup_one(context, failure_message) do
      ...
      {:error, reason} ->
        Logger.error(
          "Failed to wake flow context #{context_id} for org=#{organization_id}: #{Glific.SafeLog.safe_inspect(reason)}"
        )

        {:error,
         "Failed to wake flow context #{context_id}: #{Glific.SafeLog.safe_inspect(reason)}"}
    end
  end
