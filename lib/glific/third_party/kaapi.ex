defmodule Glific.ThirdParty.Kaapi do
  ...
  @doc """
  Initiates async WhatsApp template body rephrasing via Kaapi unified LLM API.

  Receives an already-built payload (from `Glific.TemplateRephrase.build_llm_payload/5`)
  and dispatches it to Kaapi. On success, extracts and returns `{:ok, %{job_id: job_id}}`
  for the caller to persist. On failure, logs the exception and returns `{:error, reason}`.

  ## Parameters

    - `payload` — the LLM call envelope built by `Glific.TemplateRephrase.build_llm_payload/5`.
    - `organization_id` — used to look up the Kaapi API key via `fetch_kaapi_creds/1`.
  """
  @spec rephrase_template_body(map(), non_neg_integer()) ::
          {:ok, %{job_id: String.t()}} | {:error, any()}
  def rephrase_template_body(payload, organization_id) do
    with {:ok, secrets} <- fetch_kaapi_creds(organization_id),
         {:ok, body} <- ApiClient.call_llm(payload, secrets["api_key"]) do
      case body do
        %{data: %{job_id: job_id}} when is_binary(job_id) ->
        ...
        other ->
          Glific.log_exception(%Error{
            message:
              "Kaapi rephrase_template_body returned unexpected body for org_id=#{organization_id}, body=#{safe_inspect(other)}"
          })
      ...
      {:error, reason} ->
        Glific.log_exception(%Error{
          message:
            "Kaapi rephrase_template_body failed for org_id=#{organization_id}, reason=#{safe_inspect(reason)}"
        })
