defmodule Glific.Flows do
  ...
  def start_contact_flow(%Flow{} = flow, %Contact{} = contact) do
    {:ok, flow} = get_cached_flow(contact.organization_id, {:flow_uuid, flow.id}, %{id: flow.id})
  ...
  def start_group_flow(%Flow{} = flow, %Group{} = group) do
    {:ok, flow} = get_cached_flow(group.organization_id, {:flow_uuid, flow.id}, %{id: flow.id})
