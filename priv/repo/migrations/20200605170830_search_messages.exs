...
$$ LANGUAGE plpgsql SECURITY DEFINER;
    ...
    execute("""
    CREATE OR REPLACE FUNCTION update_search_message(
      contact_id numeric,
      label text,
      message text)
    RETURNS BOOLEAN AS
  ...
  def search_messages do
    ...
    execute("""
    CREATE INDEX search_messages_index ON search_messages using gin(document)
    """)
  ...
  defp create_triggers do
    ...
    ON contacts
    FOR EACH STATEMENT
    EXECUTE PROCEDURE update_search_message(1, NULL, NULL)
    """)

    execute("""
    CREATE TRIGGER update_search_message
    AFTER INSERT OR UPDATE OR DELETE OR TRUNCATE
    ON messages
    FOR EACH STATEMENT
    EXECUTE PROCEDURE update_search_message(1, "", "");
    """)
  ...
  def down do
    execute("DROP TRIGGER IF EXISTS update_search_message ON contacts")
    execute("DROP TRIGGER IF EXISTS update_search_message ON messages")
    execute("DROP TRIGGER IF EXISTS update_search_message ON tags")
    execute("DROP TRIGGER IF EXISTS update_search_message ON messages_tags")
