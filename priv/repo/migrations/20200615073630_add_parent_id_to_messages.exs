  ...
  def up do
    ...
    BEGIN
      IF (TG_OP = 'INSERT') THEN
        ...
        UPDATE messages SET ancestors = (Array(SELECT parent_id from messages where contact_id = NEW.contact_id and id < NEW.id and parent_id is not NULL ORDER BY id DESC LIMIT #{
      @ancestors_limit
    }))
        where id = NEW.id;
