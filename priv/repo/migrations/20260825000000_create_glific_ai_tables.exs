defmodule Glific.Repo.Migrations.CreateGlificAiTables do
  ...
  Plus `glific_ai_org_profiles`, the standing instructions applied to every answer.
  ...
  def down do
    drop_if_exists(table(:glific_ai_org_profiles))
  ...
  defp create_org_profiles do
    create table(:glific_ai_org_profiles,
             comment:
               "Standing instructions for one organization, applied to every answer. For example: low-literacy audience, keep messages under 40 words."
           ) do
      add :content, :text,
        null: false,
        default: "",
        comment:
          "Free text, expected to stay in the low thousands of characters. Live reads always beat this: never record here what a tool can look up."

      add :updated_by_id, references(:users, on_delete: :nilify_all),
        comment: "Who last edited the profile"
    ...
    create unique_index(:glific_ai_org_profiles, [:organization_id],
             comment: "One profile per organization"
           )
  end
end
