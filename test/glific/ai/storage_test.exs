defmodule Glific.AI.StorageTest do
  ...
  test "an organization has at most one Glific AI profile" do
    attrs = %{organization_id: 1, content: "low-literacy audience, keep messages under 40 words"}

    {:ok, profile} = %OrgProfile{} |> OrgProfile.changeset(attrs) |> Repo.insert()
    assert profile.content =~ "low-literacy"

    assert {:error, changeset} = %OrgProfile{} |> OrgProfile.changeset(attrs) |> Repo.insert()
