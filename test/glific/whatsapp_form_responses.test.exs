defmodule Glific.WhatsappFormResponsesTest do
  ...
  test "create_whatsapp_form_response/1 creates a response successfully and write to google sheet if it exists",
    ...
    with_mock(
      ...
      # Test prepare_row_from_headers with mocked GoogleSheets.get_headers
      expected_headers = [
        "timestamp",
      ...
      with_mock(
        ...
        {:ok, ordered_row} =
          WhatsappFormsResponses.prepare_row_from_headers(@valid_attrs, "1234678904004")

        assert length(ordered_row) == length(expected_headers)

        assert Enum.at(ordered_row, 1) == to_string(@valid_attrs.contact_number)
        assert Enum.at(ordered_row, 2) == to_string(@valid_attrs.whatsapp_form_id)
      end
    end
  end
end
