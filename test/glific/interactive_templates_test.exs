defmodule Glific.InteractiveTemplatesTest do
  ...
  setup_all do
    Tesla.Mock.mock_global(&default_translate_mock/1)
    :ok
  end
  ...
  test "translate_interactive_template/1 returns an error for location-request templates on a hard Google Translate failure",
       %{organization_id: _organization_id} = attrs do
    interactive = Fixtures.interactive_fixture(Map.merge(@valid_location_attrs, attrs))
  ...
  describe "blocks" do
    @blocks_envelope %{
      ...
      "props" => %{
        "id" => "course",
        "body" => %{"kind" => "text", "value" => "please share your location"},
        "options" => %{
          "kind" => "list",
          "value" => [
            %{
    ...
    @blocks_attrs %{
      label: "Blocks Fixture",
      type: :blocks,
      language_id: 1,
      interactive_content: @blocks_envelope
    }

    test "get_interactive_body/1 returns the derived body — only text nodes, none skipped for a single-text envelope" do
      opaque_envelope = %{
        "type" => "blocks",
        "version" => 1,
        "component" => "tap/location-prompt",
        "props" => %{"body" => %{"kind" => "text", "value" => "please share your location"}}
      }

      assert InteractiveTemplates.get_interactive_body(opaque_envelope) ==
               "please share your location"
    end

    test "create_interactive_template/1 rejects an invalid blocks envelope", attrs do
      invalid_attrs =
        @blocks_attrs
        |> Map.put(:interactive_content, Map.delete(@blocks_envelope, "component"))
        |> Map.merge(attrs)
    ...
    test "create_interactive_template/1 accepts a valid blocks envelope", attrs do
      valid_attrs = Map.merge(@blocks_attrs, attrs)
    ...
    test "translate_interactive_template/1 translates every text/alt node, reinserted by path, leaving structural keys untouched",
      ...
      assert Map.has_key?(translations, "2")
      assert translations["2"]["component"] == "glific/image-panel"
      assert translations["2"]["type"] == "blocks"
      assert translations["2"]["props"]["body"]["value"] != "please share your location"
      assert translations["2"]["props"]["body"]["kind"] == "text"

      translated_option = hd(translations["2"]["props"]["options"]["value"])
      assert translated_option["id"] == "c1"
      assert translated_option["image"]["value"] == "https://example.com/1.png"
      assert translated_option["label"]["value"] != "Spoken English"
    end

    test "export_interactive_template/2 skips blocks templates and reports the skip, rather than raising",
         %{organization_id: _organization_id} = attrs do
      interactive = Fixtures.interactive_fixture(Map.merge(@blocks_attrs, attrs))

      assert {:ok, %{export_data: export_data}} =
               InteractiveTemplates.export_interactive_template(interactive, true)

      assert export_data =~ "do not support CSV export/import"
    end

    test "import_interactive_template/2 rejects blocks templates rather than raising", attrs do
      interactive = Fixtures.interactive_fixture(Map.merge(@blocks_attrs, attrs))
      ...
      assert reason =~ "do not support CSV export/import"
    end
  end
end
