defmodule Glific.Flows.Webhooks.Core.CallbackClassifierTest do
  use ExUnit.Case, async: true

  alias Glific.Flows.Webhooks.{CallbackClassifier, ErrorType}

  describe "classify/1" do
    test "a nested http_status 4xx (except 408/429) is a config error" do
      assert CallbackClassifier.classify(%{"http_status" => 400}) == :invalid_input
      assert CallbackClassifier.classify(%{"http_status" => 404}) == :invalid_input
    ...
    test "a 4xx code parsed from the reason string is a config error" do
      ...
      assert CallbackClassifier.classify(result) == :invalid_input
    end

    test "a Status: NNN code embedded in the reason is honoured too" do
      assert CallbackClassifier.classify(%{"reason" => "denied. Status: 403"}) == :invalid_input
    end

    test "a nested http_status 5xx is a system error" do
      assert CallbackClassifier.classify(%{"http_status" => 500}) == :unknown
      assert CallbackClassifier.classify(%{"http_status" => 502}) == :unknown
    ...
    test "a 5xx code parsed from the reason string is a system error" do
      ...
      assert CallbackClassifier.classify(result) == :unknown
    end

    test "408 / 429 are upstream blips → system (we do not retry)" do
      assert CallbackClassifier.classify(%{"http_status" => 408}) == :rate_limited
      assert CallbackClassifier.classify(%{"http_status" => 429}) == :rate_limited
    ...
    test "an overloaded / locked upstream is a system error (before any status rule)" do
      ...
      assert CallbackClassifier.classify(locked) == :service_unavailable

      overloaded = %{"reason" => "The server is overloaded, please try again"}
      assert CallbackClassifier.classify(overloaded) == :service_unavailable
      assert ErrorType.class(:service_unavailable) == :system
    end

    test "the error key is used when reason is absent" do
      assert CallbackClassifier.classify(%{"error" => "denied (code: 401)"}) == :invalid_input
    end
    ...
    test "a non-binary reason does not crash the classifier and fails safe to system" do
      assert CallbackClassifier.classify(%{"reason" => %{nested: "map"}}) == :unknown
      assert CallbackClassifier.classify(%{}) == :unknown
      assert CallbackClassifier.classify(nil) == :unknown
    end
