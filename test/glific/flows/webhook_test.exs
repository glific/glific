defmodule Glific.Flows.WebhookTest do
  ...
  describe "webhook logs" do
    ...
    test "list_webhook_logs/2", attrs do
      ...
      assert [Map.merge(webhook_log, %{status: "Success"})] == WebhookLog.list_webhook_logs(%{filter: attrs})
    end
    ...
    test "list_webhook_logs/2 returns filtered logs", attrs do
      ...
      assert [Map.merge(webhook_log_2, %{status: "Error"}), Map.merge(webhook_log_1, %{status: "Success"})] ==
