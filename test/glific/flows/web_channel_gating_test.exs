defmodule Glific.Flows.WebChannelGatingTest do
  @moduledoc """
  Tests for the Phase 5 web-channel capability gating: a flow context whose
  triggering message arrived over the web channel is restricted to plain-text
  sends. Covers both the runtime safety net (Glific.Flows.Action.execute/3)
  and the flow-validation gate (Glific.Flows.Flow.web_channel_capability_errors/2).
  """
  use Glific.DataCase
  ...
  describe "runtime gating (Action.execute/3) for web channel flow contexts" do
    test "send_interactive_msg is blocked and a notification is created", attrs do
      ...
      action = %Action{
        ...
        text: "Interactive message on web",
  ...
  describe "validation-time gating (Flow.validate_flow/3) for :web_message flows" do
    test "publishing a :web_message flow with interactive/broadcast actions fails validation",
         attrs do
