defmodule GlificWeb.Schema.ConversationTest do
  ...
  test "conversations always returns a few threads" do
    {:ok, result} = query_gql_by(:list, variables: %{"contactOpts" => %{"limit" => 1}, "messageOpts" => %{"limit" => 3}})
    assert get_in(result, [:data, "conversations"]) |> length >= 1

    contact_id = get_in(result, [:data, "conversations", Access.at(0), "contact", "id"])

    {:ok, result} =
      query_gql_by(:list, variables: %{"contactOpts" => %{"limit" => 1}, "messageOpts" => %{"limit" => 1}, "filter" => %{"id" => contact_id}})
  ...
  test "conversations filtered by a contact id" do
    ...
    {:ok, result} =
      query_gql_by(:list,
        variables: %{"contactOpts" => %{"limit" => 3}, "messageOpts" => %{"limit" => 3}, "filter" => %{"Gid" => "234567893453"}}
      )
    ...
    {:ok, result} =
      query_gql_by(:list, variables: %{"contactOpts" => %{"limit" => 1}, "messageOpts" => %{"limit" => 1}, "filter" => %{"id" => cid}})
  ...
  test "conversation by id" do
    ...
    {:ok, result} =
      query_gql_by(:by_contact_id, variables: %{"contact_id" => cid, "messageOpts" => %{"limit" => 1}, "filter" => %{}})
    ...
    {:ok, result} =
      query_gql_by(:by_contact_id,
        variables: %{"contact_id" => "234567893453", "messageOpts" => %{"limit" => 3}, "filter" => %{}}
      )
  ...
  test "conversation by ids" do
    ...
    {:ok, result} =
      query_gql_by(:list, variables: %{"contactOpts" => %{"limit" => 1}, "messageOpts" => %{"limit" => 1}, "filter" => %{"ids" => contact_ids}})
