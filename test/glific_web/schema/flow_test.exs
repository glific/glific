defmodule GlificWeb.Schema.FlowTest do
  ...
  test "flow get returns a flow contact",
    ...
    assert {:ok, query_data} = result
    assert String.contains?(get_in(query_data, [:data, "flowGet", "flow", "name"]), "Help Workflow")

    user = Map.put(user, :fingerprint, Ecto.UUID.generate())
    result = auth_query_gql_by(:flow_get, user, variables: %{"id" => 1})
    assert {:ok, query_data} = result
    assert get_in(query_data, [:data, "flowGet", "errors", Access.at(0), "message"]) == "The flow is being edited by some name"
