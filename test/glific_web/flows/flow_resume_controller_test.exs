defmodule GlificWeb.Flows.FlowResumeControllerTest do
  ...
  describe "flow_resume_routes" do
    ...
    test "maybe_upload_tts_audio marks tts_upload_error when the audio upload fails" do
      ...
      result = FlowResumeController.maybe_upload_tts_audio(response)

      assert is_nil(result["message"])
      assert result["tts_upload_error"] =~ "TTS audio upload failed"
    end

    test "do_flow_resume records a failure on the webhook log when TTS audio upload failed", %{
      conn: %{assigns: %{organization_id: organization_id}} = _conn
    } do
      contact = Fixtures.contact_fixture()
      webhook_log = Fixtures.webhook_log_fixture(%{organization_id: organization_id})
      ...
      # The shape maybe_upload_tts_audio/1 produces after a failed GCS upload: a Kaapi
      # success callback whose audio could not be uploaded.
      response = %{
        "organization_id" => organization_id,
        "flow_id" => flow.id,
        "contact_id" => contact.id,
