defmodule GlificWeb.Resolvers.AIEvaluationsTest do
  ...
  describe "get_improve_prompt/3" do
    test "returns :ready with the recommended prompt once a config version is linked", %{
      staff: user,
      organization_id: organization_id
    } do
      config_version = create_config_version(%{organization_id: organization_id})
      golden_qa = create_golden_qa_fixture(%{organization_id: organization_id})

      {:ok, _new_config_version} =
        %AssistantConfigVersion{}
        |> AssistantConfigVersion.changeset(%{
          assistant_id: config_version.assistant_config_version.assistant_id,
          prompt: "Improved prompt text",
          description: "[AI Generated] improved prompt",
          provider: "openai",
          model: "gpt-4o",
          settings: %{},
          status: :ready,
      ...
      {:ok, evaluation} =
        %AIEvaluation{}
        |> AIEvaluation.changeset(%{
          name: "test_evaluation_v2_get",
          status: :completed,
          kaapi_evaluation_id: 767,
          golden_qa_id: golden_qa.golden_qa.id,
          assistant_config_version_id: config_version.assistant_config_version.id,
          organization_id: organization_id
        })
        |> Repo.insert()
      ...
      assert {:ok, %{improve_prompt: fetched}} =
               AIEvaluations.get_improve_prompt(
                 nil,
      ...
      assert fetched.recommended_prompt == "Improved prompt text"
      assert fetched.commit_message == "[AI Generated] improved prompt"
    end
    ...
    test "returns an error when the evaluation does not exist", %{staff: user} do
      ...
      assert {:ok, %{errors: [%{message: "Evaluation not found."}]}} =
               AIEvaluations.get_improve_prompt(nil, %{evaluation_id: 999_999}, resolution)
    end
  end
